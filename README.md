...
- `GET /` : page d’accueil présentant la proposition de valeur AOMVP.
...
## Authentification & parcours utilisateur (Keycloak)

L'application est maintenant protégée avec **Spring Security + JWT (OAuth2 Resource Server)** et délègue l'authentification à **Keycloak**.

### Démarrage Keycloak via Docker Compose

```bash
docker compose up -d keycloak-db keycloak
```

- Console admin Keycloak: `http://localhost:8081/admin`
- Admin par défaut: `admin / admin`
- Realm importé automatiquement: `ao-mvp`
- Client public: `ao-mvp-api`
...
- **Mot de passe hashé**: géré nativement par Keycloak (stockage sécurisé).
- **JWT**: l'API Spring valide les tokens via `issuer-uri`/JWK de Keycloak.
- **Rôles**: `USER`, `ANALYST`, `ADMIN` propagés dans le token et appliqués côté API.
- **Vérification email**: activée au niveau du realm (`verifyEmail=true`).
- **Reset password**: activé (`resetPasswordAllowed=true`, flow "forgot password" Keycloak).
- **Consentement**: action requise `terms_and_conditions` activée.

### Sécurité des endpoints

- `GET /api/v1/me` : authentifié (`USER|ANALYST|ADMIN`).
- `/api/v1/users/**` et `/api/v1/notification-preferences/**` : `ADMIN` uniquement.
- Le reste de `/api/v1/**` : `USER|ANALYST|ADMIN`.

### Obtenir un token de test

```bash
curl -s -X POST 'http://localhost:8081/realms/ao-mvp/protocol/openid-connect/token'   -H 'Content-Type: application/x-www-form-urlencoded'   -d 'client_id=ao-mvp-api'   -d 'grant_type=password'   -d 'username=admin@aomvp.local'   -d 'password=Admin123!'
```
